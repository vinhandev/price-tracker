...
function App() {
  ...
  async function handleFetch(paramPrices: PriceProps[]) {
    setProgressValue(0);
    setLoading(true);
    try {
      for (let index = 0; index < paramPrices.length; index++) {
        const element = paramPrices[index];
    ...
    async function getData() {
      ...
      if (response.exists()) {
        if (response.data().data?.length > 0) {
          setPrices(response.data().data as PriceProps[]);
          await handleFetch(response.data().data as PriceProps[]);
        } else {
          setPrices(initData);
